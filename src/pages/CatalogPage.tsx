...
import { CATEGORIES, getCustomCategoriesForFilter, normalizeCategory, type Attraction, type Category } from '@/types/catalog'
...
export function CatalogPage() {
  ...
  const [snackbarVariant, setSnackbarVariant] = useState<SnackbarVariant>('added')
  const [snackbarMessage, setSnackbarMessage] = useState('Adicionado ao orçamento')
  ...
  const filtered = useMemo(() => {
    ...
    if (filter !== 'Todos')
      list = list.filter((i) => normalizeCategory(i.category) === normalizeCategory(filter))
  ...
  const categories: Category[] = [...CATEGORIES, ...getCustomCategoriesForFilter()]
    ...
    <div className="min-h-screen relative">
      ...
      <div className="relative z-10 w-full flex justify-center px-3 sm:px-4 lg:px-10 py-4 lg:py-10">
        ...
        <main className="flex-1 w-full mx-auto px-4 lg:px-8 py-4 pb-32 lg:pb-10 lg:pt-6 max-w-[980px]">
          ...
          {catalogState.status === 'ready' && (
            <>
              <div className="catalog-context-bar flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-gray-500 mb-3 lg:mb-4" role="status" aria-live="polite">
                <span>
                  {filtered.length === 0
              ...
              {viewMode === 'list' && filtered.length > 0 ? (
                <div ref={listRef} className="relative w-full catalog-list">
                  <div
        ...
        <div className="lg:hidden">
          <CartFooter count={cartItems.length} onOpen={() => setCartOpen(true)} />
        </div>
        </div>
      </div>
      ...
      <Suspense fallback={null}>
        <CartDrawer
          ...
          catalogItems={catalogState.status === 'ready' ? catalogState.items ?? [] : []}
      ...
      <Suspense fallback={null}>
        <ItemDetailModal
          ...
          onRemoveFromCart={detailFromCart && detailItem ? handleRemoveFromCart : undefined}
          onOpenCart={() => setCartOpen(true)}
      ...
      <Snackbar
        ...
        onAction={snackbarVariant === 'added' ? () => setCartOpen(true) : undefined}
      />
    </div>
  )
}
